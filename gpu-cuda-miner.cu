...
__device__ static void blake2b_compress(uint64_t *const __restrict__ h, const uint64_t *const __restrict__ block)
{
	...
	v[0] = h[0] + h[4] + block[0]; v[12] = rotr64(0x510E527FADE68281 ^ v[0], 32); v[8] = 0x6a09e667f3bcc908 + v[12]; v[4] = rotr64(h[4] ^ v[8], 24);
	v[0] = v[0] + v[4] + block[1]; v[12] = rotr64(v[12] ^ v[0], 16); v[8] = v[8] + v[12]; v[4] = rotr64(v[4] ^ v[8], 63);
	v[1] = h[1] + h[5] + block[2]; v[13] = rotr64(0x9b05688c2b3e6c1f ^ v[1], 32); v[9] = 0xbb67ae8584caa73b + v[13]; v[5] = rotr64(h[5] ^ v[9], 24);
...
__global__ void __launch_bounds__(blocksize) nonceGrind(uint8_t *const __restrict__ headerIn, uint8_t *const __restrict__ hashOut, const uint8_t *const __restrict__ targ, uint8_t *const __restrict__ nonceOut)
{
	...
	*((uint32_t*)(headerIn+32)) = id;
	...
	const uint64_t *in = (uint64_t*)headerIn;
	uint64_t h[8] =
	{
